...
class CpuController extends Controller
{
    ...
    public function query(Request $request): JsonResponse
    {
        ...
        $select = ['id', 'name', 'price', 'id_disk_1', 'id_disk_2', 'id_public_port', 'id_transfer', 'id_data_center', 'active'];

        $query = Cpu::select($select);
        ...
        if ($request->has('id_data_center')) {
            if ($request->id_data_center) {
                $query->where('id_data_center', '=',  $request->id_data_center );
            }
        }
        ...
        if ($filteredResults) {
            ...
            $success = true;
            $message = 'CPUs fetched correctly';
        }
